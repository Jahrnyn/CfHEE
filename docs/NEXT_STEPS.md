...
- the first public versioned routing shell now includes basic system, scope-values, and document-ingest endpoints, so the next leverage comes from extending that shell carefully rather than adding more internal UI or copilot-style behavior
...
2. Tighten the new public ingest-facing contract where useful:
